...
<div class="input_field">
    ...
    <input type="text" id="reg_email" placeholder="Email {if ($config->conf_us == 1)}(required){/if}">
...
<span id="reg_message_result"></span>

<div class="sign_in_on">
    <button class="submit_button" onclick="registration()">Log in</button>

    <span>{message}</span>
    <span class="create_account">Don’t have an account?
        <a id="change_to_login_window" style="cursor:pointer;">Sign on</a>
    </span>
</div>
</div>