...
// недавние плейлисты
$db_response = pdo()->prepare("SELECT tracks.id, tracks.title, tracks.cover, tracks.path, users.id AS author_id, users.display_name FROM track__history LEFT JOIN tracks ON track__history.track_id = tracks.id LEFT JOIN users ON tracks.author = users.id WHERE track__history.user_id = :user_id ORDER BY track__history.id DESC LIMIT 6");
$db_response->setFetchMode(PDO::FETCH_OBJ);
$db_response->execute([':user_id' => $user_id]);

while ($recent_playlists = $db_response->fetch()) {
    $copy_link = str_replace('files/tracks/', '', $recent_playlists->path);
    ...
    tpl()->set('{playlist_path}', $recent_playlists->path);
    tpl()->set('{script}', "player_play(`$recent_playlists->path`)");
    tpl()->set('{script_type}', "track");
    tpl()->set('{script_data}', "{name: `$copy_link`}");
    tpl()->set('{title}', "Go to track page");
    tpl()->set('{author}', 'true');
...
// пролайканые треки
$db_response = pdo()->prepare("SELECT tracks.id, tracks.cover, tracks.path, tracks.title, users.id as author_id, users.display_name FROM users__favorite_actions LEFT JOIN tracks ON users__favorite_actions.track_id = tracks.id LEFT JOIN users ON users__favorite_actions.user_id = users.id WHERE users__favorite_actions.user_id = :user_id AND users__favorite_actions.track_id IS NOT NULL ORDER BY users__favorite_actions.id DESC LIMIT 6");
$db_response->setFetchMode(PDO::FETCH_OBJ);
$db_response->execute([':user_id' => $user_id]);

while ($likes = $db_response->fetch()) {
    $copy_link = str_replace('files/tracks/', '', $likes->path);
    $copy_link = substr($copy_link, 0, strrpos($copy_link, '.'));
    ...
    tpl()->set('{script}', "player_play(`$likes->path`)");
    tpl()->set('{script_type}', "track");
    tpl()->set('{script_data}', "{name: `$copy_link`}");
    tpl()->set('{title}', "Go to track page");
    tpl()->set('{author}', 'true');
