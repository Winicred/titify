<div class="profile_data_item" data-bs-toggle="tooltip" title="Go to track page" data-bs-placement="left"
     onclick="redirect_blank_page('{track_url}', $(this))">
    <div class="item_image">
        <div class="cover_image"
             style="background-image: url('{cover}'); width: 160px; height: 160px; background-repeat: no-repeat; background-position: center; background-size: contain"
             onclick="redirect_blank_page('{track_url}', $(this))"></div>
    </div>
            ...
            {if (is_auth())}
                <div class="comment_field">
                    ...
                    <input type="text" placeholder="Write a comment" track-id="{id}"
                           title="Send track comment (press enter to upload)" data-bs-toggle="tooltip"
                           data-bs-placement="left">
                </div>
            {/if}

            <div class="item_stats">
                <button class="likes {if('{is_liked}' == 'true')}active{/if}" onclick="add_favorite_track({id})"
                        data-id="{id}" title="Click to like the track" data-bs-toggle="tooltip"
                        data-bs-placement="bottom">
                    <i class="fa-solid fa-heart"></i>
                    <span>{likes}</span>
                </button>

                <button class="comments" data-id="{id}" title="Comments count" data-bs-toggle="tooltip"
                        data-bs-placement="bottom">
                ...
                <button onclick="copy_to_clipboard('{track_url}', $(this))" title="Click to copy track link"
                ...
                {if (is_auth())}
                    <button onclick="call_modal('playlist_action', {track_id: {id}})"
                            title="Click to add track to the playlist" data-bs-toggle="tooltip"
                            data-bs-placement="bottom">
